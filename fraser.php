...
if(isset($_POST['book'])){
   ...
   if($total_rooms >= 30){
      ...
      $verify_bookings = $conn->prepare("SELECT * FROM `bookings` WHERE user_id = ? AND name = ? AND email = ? AND number = ? AND rooms = ? AND roomtype = ? AND hotel = ? AND check_in = ? AND check_out = ? AND adults = ? AND childs = ?");
      $verify_bookings->execute([$user_id, $name, $email, $number, $rooms, $roomtype, $hotel, $check_in, $check_out, $adults, $childs]);

      if($verify_bookings->rowCount() > 0){
         $warning_msg[] = 'room booked alredy!';
      }else{
         $book_room = $conn->prepare("INSERT INTO `bookings`(booking_id, user_id, name, email, number, rooms, roomtype,hotel, check_in, check_out, adults, childs) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
         $book_room->execute([$booking_id, $user_id, $name, $email, $number, $rooms, $roomtype,$hotel, $check_in, $check_out, $adults, $childs]);
...
<body style="overflow-y: scroll;height: 800px;overflow-x: hidden;overflow:hidden;color:grey;background-color:white">
...
<div class="banner-top">
		...
		<ul>
		  ...
		  <div id="wrapper" class="toggled">
			<!-- /#wrapper -->
			 <img class="carbon-notification" src="goa.jpg" style=" position: absolute;
			 width: 50px;
			 height: 32px;
			 top: 12px;
			 left: 1150px;" />
...
<img src="./uploads/singapore.jpg"style=" width:110em;  height:35em;">
...
<form action="" method="post">
   ...
   <input type="submit" value="check availability" name="check" class="btn" style="    background-color: #007bff;
    box-shadow: inset 0 0em 0em #1bdc74, 0 0 0 -2px #abacab, 0.3em 0.3em 1em #1387e775;">
</form>
