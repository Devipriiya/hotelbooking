...
if(isset($_POST['book'])){
   ...
   $numberofnights = filter_var($numberofnights, FILTER_SANITIZE_STRING);
   $amount = $_POST['amount'];
   ...
   if($total_rooms >= 30){
      ...
      $verify_bookings = $conn->prepare("SELECT * FROM `bookings` WHERE user_id = ? AND name = ? AND email = ? AND number = ? AND rooms = ? AND numberofnights = ? AND amount = ? AND hotel = ? AND check_in = ? AND check_out = ? AND adults = ? AND childs = ?");
      $verify_bookings->execute([$user_id, $name, $email, $number, $rooms, $numberofnights, $amount, $hotel, $check_in, $check_out, $adults, $childs]);

      if($verify_bookings->rowCount() > 0){
         $warning_msg[] = 'room booked alredy!';
      }else{
         $book_room = $conn->prepare("INSERT INTO `bookings`(booking_id, user_id, name, email, number, rooms,numberofnights, amount, hotel, check_in, check_out, adults, childs) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)");
         $book_room->execute([$booking_id, $user_id, $name, $email, $number, $rooms, $numberofnights, $amount, $hotel, $check_in, $check_out, $adults, $childs]);
    ...
    box-shadow: inset 0 0em 0em #1bdc74, 0 0 0 -2px #abacab, 0.3em 0.3em 1em #1387e775;">make a reservation</h3>
      <div class="flex">
         <div class="box">
            ...
            <input type="text" name="name"  id="name" maxlength="50" required placeholder="enter your name" class="input">
         </div>
         <div class="box">
            <p>your email <span>*</span></p>
            <input type="email" name="email" id="email" maxlength="50" required placeholder="enter your email" class="input">
         </div>
         ...
         <div class="box">
            ...
            <input type="hotel" name="hotel" id="hotel" maxlength="50" required placeholder="enter hotel name" class="input">
         </div>
         <div class="box">
            <p>your number <span>*</span></p>
            <input type="number" name="number" id="number"  maxlength="10" min="0" max="9999999999" required placeholder="enter your number" class="input">
         </div>
         <div class="box">
            <p>rooms <span>*</span></p>
            <select id="roomType" name="rooms" required style="    width: 100%;
    height: 42px;">
         ...
         <div class="box">
         <p>NumberofNights <span>*</span></p>

         <input type="number" id="numOfNights" name="numberofnights" min="1" value="1" required style="    width: 100%;
    height: 42px;">
        </div>
<div class="box">
    ...
    height: 42px;">
     ...
     <div class="box">
            ...
            <input type="date" name="check_in" id="checkIn" class="input" required>
         </div>
         <div class="box">
            <p>check out <span>*</span></p>
            <input type="date" name="check_out"id="checkOut" class="input" required>
         </div>
         <div class="box">
            <p>adults <span>*</span></p>
            <select name="adults" class="input" id="adults" required>
         ...
         <div class="box">
            ...
            <select name="childs" class="input" id="childs" required>
    ...
    box-shadow: inset 0 0em 0em #1bdc74, 0 0 0 -2px #abacab, 0.3em 0.3em 1em #1387e775;"> -->
      <button type="button" onclick="calculateAmount()" class="btn">Calculate Amount</button>
      <input type="submit" value="book now" name="book" class="btn" style="background-color: #007bff;
    box-shadow: inset 0 0em 0em #1bdc74, 0 0 0 -2px #abacab, 0.3em 0.3em 1em #1387e775;">
   </form>
